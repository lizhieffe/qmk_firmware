...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    case VIM_CLANGFORMAT:
        if (record->event.pressed) {
            ...
            SEND_STRING(SS_TAP(X_ESC)SS_LSFT(";")"ClangFormat\n");
        } else {
    ...
    case VIM_BRACLET:
        if (record->event.pressed) {
            // when keycode VIM_BRACLET is pressed
            SEND_STRING(SS_TAP(X_ESC)"a{\n\n}"SS_TAP(X_ESC)"ka");
        } else {
            // when keycode VIM_BRACLET is released
        }
        break;
    case GMAIL_DELETE:
        if (record->event.pressed) {
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  ...
  [_QWERTY] = LAYOUT_5x6(
     ...
     GMAIL_DELETE, KC_Z  , KC_X  , KC_C  , KC_V  , KC_B  ,                          KC_N  , KC_M  ,KC_COMM,KC_DOT ,KC_SLSH,KC_BSLASH,
                      CHROME_LEFT_TAB,CHROME_RIGHT_TAB,                                                       KC_LEFT, KC_RIGHT,
                                      MT(MOD_LGUI,KC_ESC),KC_LCTL,                        KC_SPC,LT(_RAISE, KC_ENT),
                                      RAISE,KC_LSFT,                         KC_BSPC,LT(_LOWER, KC_TAB),
                                      LOWER, KC_LALT,                        KC_DEL,KC_RBRC
  ),

  [_RAISE] = LAYOUT_5x6(
       KC_F12 , KC_F1 , KC_F2 , KC_F3 , KC_F4 , KC_F5 ,                        KC_F6  , KC_F7 , KC_F8 , KC_F9 ,KC_F10 ,KC_F11 ,
       KC_MS_ACCEL0,_______,_______,KC_MS_UP,_______,KC_LBRC,                        KC_RBRC,KC_PGDOWN,KC_PGUP,KC_INS ,KC_SLCK,KC_MUTE,
       KC_MS_ACCEL1,_______,KC_MS_LEFT,KC_MS_DOWN,KC_MS_RIGHT,KC_MS_BTN1,                        KC_LEFT,KC_DOWN,KC_UP,KC_RIGHT,_______,KC_VOLU,
       KC_MS_ACCEL2,_______,KC_MS_WH_LEFT,_______,KC_MS_WH_RIGHT,KC_MS_BTN2,                        VIM_BRACLET,VIM_CLANGFORMAT,_______,_______,_______,KC_VOLD,
